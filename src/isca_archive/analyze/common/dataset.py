...
class ISCAArchiveProcessorDataset(Dataset):
    ...
    def __init__(
        ...
        list_conference: list[str] | None = None,
        load_full_text: bool = False,
        use_isca_keywords: bool = False,
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if False: # TODO: this is diabled for now as it is unclear on how to manage this
            ...
            try:
                ...
                self._logger.warning(f'The submission "{paper_id}" doesn\'t have any area assigned: {ex}')
    ...
    def _get_area(self, conference: str, paper: dict[str, Any]) -> str | None:
        ...
        df_conf_areas = pd.read_csv(conf_resource, sep="\t", dtype={"paper_id": str})
...
class ISCAArchiveProcessedDataset(Dataset):
    ...
    def __init__(
        ...
        if years is not None:
            ...
            #
        self.df = self.df.loc[
            (self.df.author_area_id != "")
            & (self.df.author_area_id != "Show and Tell")
            & (~pd.isna(self.df.author_area_id))
        ]
