...
def add_subparsers(subparsers):
    ...
    parser.add_argument(
        ...
        "--full-text",
        action="store_true",
        help="Load full text (converted from PDF file, so it can be dirty) instead of abstract",
    )
    parser.add_argument(
        "-k",
        "--isca-keywords",
        action="store_true",
        help="Only load the ISCA Archive keywords extracted during the publication phase (incompatible with full text)",
    )
...
def main(args: argparse.Namespace):
    ...
    dataset = ISCAArchiveProcessorDataset(
        pathlib.Path(args.isca_archive_root),
        list_conf,
        load_full_text=args.full_text,
        use_isca_keywords=args.isca_keywords,
